  ...
  shinyServer(function(input, output, session) {
    ...
    plotdata_ViolinPlot_low <- reactive({
      ...
      VlnPlot_scCustom(seurat_object = get(rv_depotgene$depot), features = rv_depotgene$gene) + 
    ...
    plotdata_FeaturePlot_high <- reactive({
      ...
      p1 <- DimPlot_scCustom(seurat_object = get(rv_genesubcluster$subcluster))[[1]] + theme(aspect.ratio=1, text = element_text(family = "Red Hat Display")) + NoAxes() + NoLegend()
    ...
    plotdata_ViolinPlot_high <- reactive({
      ...
      VlnPlot_scCustom(seurat_object = get(rv_genesubcluster$subcluster), features = rv_genesubcluster$gene) + 
        theme(text = element_text(family = "Red Hat Display"),
              axis.title.x = element_blank())  # 隐藏 X 轴标题)
    })
