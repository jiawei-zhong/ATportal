      ...
      if (input$mode == "normalized") {
        if (nrow(data_hyp_heat[rownames(data_hyp_heat) %in% rownames(v$hyp),]) == 1) {
          v$heatmap_hyp <- pheatmap((data_hyp_heat[rownames(data_hyp_heat) %in% rownames(v$hyp),]), 
                                ...
                                main = "Hypoxia", labels_row = v$hyp$GENE ,silent = T)
        } else {
          v$heatmap_hyp <- pheatmap((data_hyp_heat[rownames(data_hyp_heat) %in% rownames(v$hyp),]), 
                                scale = "row", cluster_cols = TRUE, 
                                color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
                                border_color = NA, main = "Hypoxia", labels_row = v$hyp$GENE ,silent = T)
        }
      } else {
        if (nrow(data_hyp_heat[rownames(data_hyp_heat) %in% rownames(v$hyp),]) == 1) {
          v$heatmap_hyp <- pheatmap((data_hyp_heat[rownames(data_hyp_heat) %in% rownames(v$hyp),]), 
       ...
       if (input$mode == "normalized") {
         if (nrow(data_tnf_heat[data_tnf_heat$gene %in% v$tnf$gene,1:6]) == 1) {
           v$heatmap_tnf <- pheatmap((data_tnf_heat[data_tnf_heat$gene %in% v$tnf$gene,1:6]), 
                                     scale = "row", cluster_cols = FALSE, cluster_rows= FALSE, 
                                     color = farben[[input$gradient_col]], border_color = NA, 
                                     main = "TNF stimulation", labels_row = data_tnf_heat[data_tnf_heat$gene %in% v$tnf$gene,7] ,silent = T)
         } else {
           v$heatmap_tnf <- pheatmap((data_tnf_heat[data_tnf_heat$gene %in% v$tnf$gene,1:6]), 
                                     scale = "row", cluster_cols = TRUE, 
                                     color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
                                     border_color = NA, main = "TNF stimulation", labels_row = data_tnf_heat[data_tnf_heat$gene %in% v$tnf$gene,7] ,silent = T)
         }
       } else {
         if (nrow(data_tnf_heat[data_tnf_heat$gene %in% v$tnf$gene,1:6]) == 1) {
           v$heatmap_tnf <- pheatmap((data_tnf_heat[data_tnf_heat$gene %in% v$tnf$gene,1:6]), 
       ...
       if (input$mode == "normalized") {
         if (nrow(v$stim) == 1) {
           v$heatmap_tnf <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "TNFa", arr.ind = TRUE)], 
                                     ...
                                     labels_col = data_stim_group[which(data_stim_group$group == "Control" | data_stim_group$group == "TNFa"),2] ,silent = T)
         } else {
           v$heatmap_tnf <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "TNFa", arr.ind = TRUE)],
                                     scale = "row", cluster_cols = TRUE, 
                                     color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
         ...
         if (input$mode == "normalized") {
           if (nrow(data_ins_heat[rownames(data_ins_heat) %in% v$ins$genes,]) == 1) {
             v$heatmap_ins <- pheatmap((data_ins_heat[rownames(data_ins_heat) %in% v$ins$genes,]), 
                                       scale = "row", cluster_cols = FALSE, cluster_rows= FALSE, 
                                       color = farben[[input$gradient_col]], border_color = NA, 
                                       main = "Insulin", labels_row = v$ins$genes ,silent = T)
           } else {
             v$heatmap_ins <- pheatmap((data_ins_heat[rownames(data_ins_heat) %in% v$ins$genes,]), 
                                       scale = "row", cluster_cols = TRUE, 
                                       color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
                                       border_color = NA, main = "Insulin", labels_row = v$ins$genes ,silent = T)
           }
         } else {
           if (nrow(data_ins_heat[rownames(data_ins_heat) %in% v$ins$genes,]) == 1) {
             v$heatmap_ins <- pheatmap((data_ins_heat[rownames(data_ins_heat) %in% v$ins$genes,]), 
         ...
         if (input$mode == "normalized") {
           if (nrow(v$stim) == 1) {
             v$heatmap_ins <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "Insulin", arr.ind = TRUE)], 
                                       ...
                                       labels_col = data_stim_group[which(data_stim_group$group == "Control" | data_stim_group$group == "Insulin"),2] ,silent = T)
           } else {
             v$heatmap_ins <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "Insulin", arr.ind = TRUE)],
                                       scale = "row", cluster_cols = TRUE, 
                                       color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
     ...
     if (input$mode == "normalized") {
       if (nrow(v$stim) == 1) {
         v$heatmap_il6 <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "IL6", arr.ind = TRUE)], 
                                   ...
                                   labels_col = data_stim_group[which(data_stim_group$group == "Control" | data_stim_group$group == "IL6"),2] ,silent = T)
       } else {
         v$heatmap_il6 <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "IL6", arr.ind = TRUE)],
                                   scale = "row", cluster_cols = TRUE, 
                                   color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
     ...
     if (input$mode == "normalized") {
       if (nrow(v$stim) == 1) {
         v$heatmap_tgf <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "TGFB1", arr.ind = TRUE)], 
                                   ...
                                   labels_col = data_stim_group[which(data_stim_group$group == "Control" | data_stim_group$group == "TGFB1"),2] ,silent = T)
       } else {
         v$heatmap_tgf <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "TGFB1", arr.ind = TRUE)],
                                   scale = "row", cluster_cols = TRUE, 
                                   color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
     ...
     if (input$mode == "normalized") {
       if (nrow(v$stim) == 1) {
         v$heatmap_rosi <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "Rosiglitazone", arr.ind = TRUE)], 
                                   ...
                                   labels_col = data_stim_group[which(data_stim_group$group == "Control" | data_stim_group$group == "Rosiglitazone"),2] ,silent = T)
       } else {
         v$heatmap_rosi <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "Rosiglitazone", arr.ind = TRUE)],
                                   scale = "row", cluster_cols = TRUE, 
                                   color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
     ...
     if (input$mode == "normalized") {
       if (nrow(v$stim) == 1) {
         v$heatmap_dexa <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "Dexamethasone", arr.ind = TRUE)], 
                                   ...
                                   labels_col = data_stim_group[which(data_stim_group$group == "Control" | data_stim_group$group == "Dexamethasone"),2] ,silent = T)
       } else {
         v$heatmap_dexa <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "Dexamethasone", arr.ind = TRUE)],
                                   scale = "row", cluster_cols = TRUE, 
                                   color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
     ...
     if (input$mode == "normalized") {
       if (nrow(v$stim) == 1) {
         v$heatmap_iso <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "Isoprenaline", arr.ind = TRUE)], 
                                   ...
                                   labels_col = data_stim_group[which(data_stim_group$group == "Control" | data_stim_group$group == "Isoprenaline"),2] ,silent = T)
       } else {
         v$heatmap_iso <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "Isoprenaline", arr.ind = TRUE)],
                                   scale = "row", cluster_cols = TRUE, 
                                   color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
     ...
     if (input$mode == "normalized") {
       if (nrow(v$stim) == 1) {
         v$heatmap_ibmx <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "IBMX", arr.ind = TRUE)], 
                                   ...
                                   labels_col = data_stim_group[which(data_stim_group$group == "Control" | data_stim_group$group == "IBMX"),2] ,silent = T)
       } else {
         v$heatmap_ibmx <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "IBMX", arr.ind = TRUE)],
                                   scale = "row", cluster_cols = TRUE, 
                                   color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
     ...
     if (input$mode == "normalized") {
       if (nrow(v$stim) == 1) {
         v$heatmap_met <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "Metformin", arr.ind = TRUE)], 
                                   ...
                                   labels_col = data_stim_group[which(data_stim_group$group == "Control" | data_stim_group$group == "Metformin"),2] ,silent = T)
       } else {
         v$heatmap_met <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "Metformin", arr.ind = TRUE)],
                                   scale = "row", cluster_cols = TRUE, 
                                   color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
     ...
     if (input$mode == "normalized") {
       if (nrow(v$stim) == 1) {
         v$heatmap_atorva <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "Atorvastatin", arr.ind = TRUE)], 
                                   ...
                                   labels_col = data_stim_group[which(data_stim_group$group == "Control" | data_stim_group$group == "Atorvastatin"),2] ,silent = T)
       } else {
         v$heatmap_atorva <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "Atorvastatin", arr.ind = TRUE)],
                                   scale = "row", cluster_cols = TRUE, 
                                   color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
     ...
     if (input$mode == "normalized") {
       if (nrow(v$stim) == 1) {
         v$heatmap_sb <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "SB203580", arr.ind = TRUE)], 
                                   ...
                                   labels_col = data_stim_group[which(data_stim_group$group == "Control" | data_stim_group$group == "SB203580"),2] ,silent = T)
       } else {
         v$heatmap_sb <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "SB203580", arr.ind = TRUE)],
                                   scale = "row", cluster_cols = TRUE, 
                                   color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
     ...
     if (input$mode == "normalized") {
       if (nrow(v$stim) == 1) {
         v$heatmap_sp <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "SP600125", arr.ind = TRUE)], 
                                   ...
                                   labels_col = data_stim_group[which(data_stim_group$group == "Control" | data_stim_group$group == "SP600125"),2] ,silent = T)
       } else {
         v$heatmap_sp <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "SP600125", arr.ind = TRUE)],
                                   scale = "row", cluster_cols = TRUE, 
                                   color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
     ...
     if (input$mode == "normalized") {
       if (nrow(v$stim) == 1) {
         v$heatmap_uo <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "UO126", arr.ind = TRUE)], 
                                   ...
                                   labels_col = data_stim_group[which(data_stim_group$group == "Control" | data_stim_group$group == "UO126"),2] ,silent = T)
       } else {
         v$heatmap_uo <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "UO126", arr.ind = TRUE)],
                                   scale = "row", cluster_cols = TRUE, 
                                   color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
     ...
     if (input$mode == "normalized") {
       if (nrow(v$stim) == 1) {
         v$heatmap_lep <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "Leptin", arr.ind = TRUE)], 
                                   ...
                                   labels_col = data_stim_group[which(data_stim_group$group == "Control" | data_stim_group$group == "Leptin"),2] ,silent = T)
       } else {
         v$heatmap_lep <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "Leptin", arr.ind = TRUE)],
                                   scale = "row", cluster_cols = TRUE, 
                                   color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
     ...
     if (input$mode == "normalized") {
       if (nrow(v$stim) == 1) {
         v$heatmap_adipo <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "Adiponectin", arr.ind = TRUE)], 
                                   ...
                                   labels_col = data_stim_group[which(data_stim_group$group == "Control" | data_stim_group$group == "Adiponectin"),2] ,silent = T)
       } else {
         v$heatmap_adipo <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "Adiponectin", arr.ind = TRUE)],
                                   scale = "row", cluster_cols = TRUE, 
                                   color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
     ...
     if (input$mode == "normalized") {
       if (nrow(v$stim) == 1) {
         v$heatmap_igf <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "IGF1", arr.ind = TRUE)], 
                                   ...
                                   labels_col = data_stim_group[which(data_stim_group$group == "Control" | data_stim_group$group == "IGF1"),2] ,silent = T)
       } else {
         v$heatmap_igf <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "IGF1", arr.ind = TRUE)],
                                   scale = "row", cluster_cols = TRUE, 
                                   color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
     ...
     if (input$mode == "normalized") {
       if (nrow(v$stim) == 1) {
         v$heatmap_glucose <- pheatmap(v$stim[,which(data_stim_group$group == "Control_Glucose" | data_stim_group$group == "Glucose", arr.ind = TRUE)], 
                                   ...
                                   labels_col = data_stim_group[which(data_stim_group$group == "Control_Glucose" | data_stim_group$group == "Glucose"),2] ,silent = T)
       } else {
         v$heatmap_glucose <- pheatmap(v$stim[,which(data_stim_group$group == "Control_Glucose" | data_stim_group$group == "Glucose", arr.ind = TRUE)],
                                   scale = "row", cluster_cols = TRUE, 
                                   color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
     ...
     if (input$mode == "normalized") {
       if (nrow(v$stim) == 1) {
         v$heatmap_lauro <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "Lauroylcarnitine", arr.ind = TRUE)], 
                                   ...
                                   labels_col = data_stim_group[which(data_stim_group$group == "Control" | data_stim_group$group == "Lauroylcarnitine"),2] ,silent = T)
       } else {
         v$heatmap_lauro <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "Lauroylcarnitine", arr.ind = TRUE)],
                                   scale = "row", cluster_cols = TRUE, 
                                   color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
     ...
     if (input$mode == "normalized") {
       if (nrow(v$stim) == 1) {
         v$heatmap_decano <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "Decanoyllcarnithine", arr.ind = TRUE)], 
                                   ...
                                   labels_col = data_stim_group[which(data_stim_group$group == "Control" | data_stim_group$group == "Decanoyllcarnithine"),2] ,silent = T)
       } else {
         v$heatmap_decano <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "Decanoyllcarnithine", arr.ind = TRUE)],
                                   scale = "row", cluster_cols = TRUE, 
                                   color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
     ...
     if (input$mode == "normalized") {
       if (nrow(v$stim) == 1) {
         v$heatmap_ra <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "RetinoicAcid", arr.ind = TRUE)], 
                                   ...
                                   labels_col = data_stim_group[which(data_stim_group$group == "Control" | data_stim_group$group == "RetinoicAcid"),2] ,silent = T)
       } else {
         v$heatmap_ra <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "RetinoicAcid", arr.ind = TRUE)],
                                   scale = "row", cluster_cols = TRUE, 
                                   color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
     ...
     if (input$mode == "normalized") {
       if (nrow(v$stim) == 1) {
         v$heatmap_wort <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "Wortmanin", arr.ind = TRUE)], 
                                   ...
                                   labels_col = data_stim_group[which(data_stim_group$group == "Control" | data_stim_group$group == "Wortmanin"),2] ,silent = T)
       } else {
         v$heatmap_wort <- pheatmap(v$stim[,which(data_stim_group$group == "Control" | data_stim_group$group == "Wortmanin", arr.ind = TRUE)],
                                   scale = "row", cluster_cols = TRUE, 
                                   color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
     ...
     if (input$mode == "normalized") {
       if (nrow(data_gls_heat[data_gls_heat$gene %in% v$gls$gene,]) == 1) {
         v$heatmap_gls <- pheatmap((data_gls_heat[data_gls_heat$gene %in% v$gls$gene,1:6]), 
                                   scale = "row", cluster_cols = FALSE, cluster_rows= FALSE, 
                                   color = farben[[input$gradient_col]], border_color = NA, 
                                   main = "gls", labels_row = v$gls$gene ,silent = T)
       } else {
         v$heatmap_gls <- pheatmap((data_gls_heat[data_gls_heat$gene %in% v$gls$gene,1:6]), 
                                   scale = "row", cluster_cols = TRUE, 
                                   color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
                                   border_color = NA, main = "gls", labels_row = v$gls$gene ,silent = T)
       }
     } else {
       if (nrow(data_gls_heat[data_gls_heat$gene %in% v$gls$gene,]) == 1) {
         v$heatmap_gls <- pheatmap((data_gls_heat[data_gls_heat$gene %in% v$gls$gene,1:6]), 
     ...
     if (input$mode == "normalized") {
       if (nrow(data_aqp7_heat[rownames(data_aqp7_heat) %in% v$aqp7$GENE,]) == 1) {
         v$heatmap_aqp7 <- pheatmap((data_aqp7_heat[rownames(data_aqp7_heat) %in% v$aqp7$GENE,]), 
                                   scale = "row", cluster_cols = FALSE, cluster_rows= FALSE, 
                                   color = farben[[input$gradient_col]], border_color = NA, 
                                   main = "aqp7", labels_row = v$aqp7$gene ,silent = T)
       } else {
         v$heatmap_aqp7 <- pheatmap((data_aqp7_heat[rownames(data_aqp7_heat) %in% v$aqp7$GENE,]), 
                                   scale = "row", cluster_cols = TRUE, 
                                   color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
                                   border_color = NA, main = "aqp7", labels_row = v$aqp7$gene ,silent = T)
       }
     } else {
       if (nrow(data_aqp7_heat[rownames(data_aqp7_heat) %in% v$aqp7$GENE,]) == 1) {
         v$heatmap_aqp7 <- pheatmap((data_aqp7_heat[rownames(data_aqp7_heat) %in% v$aqp7$GENE,]), 
                                   cluster_cols = FALSE, cluster_rows= FALSE, 
                                   color = farben[[input$gradient_col]], border_color = NA, 
                                   main = "aqp7ulin", labels_row = v$aqp7$gene ,silent = T)
       } else {
         v$heatmap_aqp7 <- pheatmap((data_aqp7_heat[rownames(data_aqp7_heat) %in% v$aqp7$GENE,]), 
                                   cluster_cols = FALSE, 
                                   color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
     ...
     if (input$mode == "normalized") {
       if (nrow(data_cd248_heat[rownames(data_cd248_heat) %in% v$cd248$GENE,]) == 1) {
         v$heatmap_cd248 <- pheatmap((data_cd248_heat[rownames(data_cd248_heat) %in% v$cd248$GENE,]), 
                                    scale = "row", cluster_cols = FALSE, cluster_rows= FALSE, 
                                    color = farben[[input$gradient_col]], border_color = NA, 
                                    main = "cd248", labels_row = v$cd248$gene ,silent = T)
       } else {
         v$heatmap_cd248 <- pheatmap((data_cd248_heat[rownames(data_cd248_heat) %in% v$cd248$GENE,]), 
                                    scale = "row", cluster_cols = TRUE, 
                                    color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
                                    border_color = NA, main = "cd248", labels_row = v$cd248$gene ,silent = T)
       }
     } else {
       if (nrow(data_cd248_heat[rownames(data_cd248_heat) %in% v$cd248$GENE,]) == 1) {
         v$heatmap_cd248 <- pheatmap((data_cd248_heat[rownames(data_cd248_heat) %in% v$cd248$GENE,]), 
                                    cluster_cols = FALSE, cluster_rows= FALSE, 
                                    color = farben[[input$gradient_col]], border_color = NA, 
                                    main = "cd248ulin", labels_row = v$cd248$gene ,silent = T)
       } else {
         v$heatmap_cd248 <- pheatmap((data_cd248_heat[rownames(data_cd248_heat) %in% v$cd248$GENE,]), 
                                    cluster_cols = FALSE, 
                                    color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
     ...
     if (input$mode == "normalized") {
       if (nrow(data_c14_heat[rownames(data_c14_heat) %in% v$c14$GENE,]) == 1) {
         v$heatmap_c14 <- pheatmap((data_c14_heat[rownames(data_c14_heat) %in% v$c14$GENE,]), 
                                     scale = "row", cluster_cols = FALSE, cluster_rows= FALSE, 
                                     color = farben[[input$gradient_col]], border_color = NA, 
                                     main = "c14", labels_row = v$c14$gene ,silent = T)
       } else {
         v$heatmap_c14 <- pheatmap((data_c14_heat[rownames(data_c14_heat) %in% v$c14$GENE,]), 
                                     scale = "row", cluster_cols = TRUE, 
                                     color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
                                     border_color = NA, main = "c14", labels_row = v$c14$gene ,silent = T)
       }
     } else {
       if (nrow(data_c14_heat[rownames(data_c14_heat) %in% v$c14$GENE,]) == 1) {
         v$heatmap_c14 <- pheatmap((data_c14_heat[rownames(data_c14_heat) %in% v$c14$GENE,]), 
                                     cluster_cols = FALSE, cluster_rows= FALSE, 
                                     color = farben[[input$gradient_col]], border_color = NA, 
                                     main = "c14ulin", labels_row = v$c14$gene ,silent = T)
       } else {
         v$heatmap_c14 <- pheatmap((data_c14_heat[rownames(data_c14_heat) %in% v$c14$GENE,]), 
                                     cluster_cols = FALSE, 
                                     color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
     ...
     if (input$mode == "normalized") {
       if (nrow(v$inflamed) == 1) {
         v$heatmap_inflAT <- pheatmap(v$inflamed[,inflamed_select()], 
                                  scale = "row", cluster_cols = FALSE, cluster_rows= FALSE, 
                                  color = farben[[input$gradient_col]], border_color = NA, 
                                  main = "Inflamed AT", labels_row = v$inflamed$SYMBOL ,silent = T)
       } else {
         v$heatmap_inflAT <- pheatmap(v$inflamed[,inflamed_select()],
                                  scale = "row", cluster_cols = TRUE, 
                                  color = farben[[input$gradient_col]], clustering_method = input$cluster_method,
                                  border_color = NA, main = "Inflamed AT", labels_row = v$inflamed$SYMBOL ,silent = T)
       }
     } else {
       if (nrow(v$inflamed) == 1) {
         v$heatmap_inflAT <- pheatmap(v$inflamed[,inflamed_select()], 
