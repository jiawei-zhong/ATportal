    ...
    if (gene_reactive()=="") {
      ...
      if (file.exists(paste0('./data/',gene_reactive(),'_RNA.RDS'))) {
        ...
        output$plot2 <- renderPlot({
          ...
          # 先处理SMD值小于-2和大于2的部分
          depots_df_RNA$SMD_binned <- ifelse(depots_df_RNA$SMD < -1, "<-1",
                                      ifelse(depots_df_RNA$SMD > 1, ">1", NA))
          
          # 处理其余部分，将其分成50个等距离的bin
          bins <- 50
          bin_breaks <- seq(-1, 1, length.out = bins + 1)
          depots_df_RNA$SMD_binned[is.na(depots_df_RNA$SMD_binned)] <- cut(depots_df_RNA$SMD[is.na(depots_df_RNA$SMD_binned)], 
                                                             breaks = bin_breaks, include.lowest = TRUE)
          
          # 计算每个bin中的计数
          bin_counts <- table(depots_df_RNA$SMD_binned)
          ...
          bin_data$group <- "out"
          bin_data$group[bin_data$start < depots_df_RNA$SMD[rownames(depots_df_RNA)==gene] & bin_data$end > depots_df_RNA$SMD[rownames(depots_df_RNA)==gene]] <- "in"
        ...
        if (file.exists(paste0('./data/',gene_reactive(),'_protein.RDS'))) {
          ...
          output$plot4 <- renderPlot({
            ...
            # 先处理SMD值小于-2和大于2的部分
            depots_df_protein$SMD_binned <- ifelse(depots_df_protein$SMD < -1, "<-1",
                                              ifelse(depots_df_protein$SMD > 1, ">1", NA))
            
            # 处理其余部分，将其分成50个等距离的bin
            bins <- 50
            bin_breaks <- seq(-1, 1, length.out = bins + 1)
            depots_df_protein$SMD_binned[is.na(depots_df_protein$SMD_binned)] <- cut(depots_df_protein$SMD[is.na(depots_df_protein$SMD_binned)], 
                                                                           breaks = bin_breaks, include.lowest = TRUE)
            
            # 计算每个bin中的计数
            bin_counts <- table(depots_df_protein$SMD_binned)
            ...
            bin_data$group <- "out"
            bin_data$group[bin_data$start < depots_df_protein$SMD[rownames(depots_df_protein)==gene] & bin_data$end > depots_df_protein$SMD[rownames(depots_df_protein)==gene]] <- "in"
            bin_data$group <- factor(bin_data$group,levels = c("in","out"))
