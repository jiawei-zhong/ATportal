...
shinyServer(function(input, output, session) {
  ...
  output$ui <- renderUI(
    if (is.null(input$data_from_html)) {
      ...
      if (input$data_from_html %in% c(rownames(DEOSHeset_WL),rownames(POeset_WL), rownames(GSE141221_diogenes1_WL), rownames(GSE95640_diogenes2_WL))) {
        
        output$plot <- renderPlot({
          if (input$data_from_html %in% rownames(DEOSHeset_WL)) {
            temp_pdat <- pData(DEOSHeset_WL)
            temp_pdat$expression <- exprs(DEOSHeset_WL)[input$data_from_html,]
            ...
            p1 <- ggboxplot(data = temp_pdat,x = "time_point",y = "expression",add = "dotplot",fill = "time_point",xlab = "years after bariatric surgery",ylab = paste0(input$data_from_html, " expression"), title = "Kerr, A. (2020)")+
          ...
          if (input$data_from_html %in% rownames(POeset_WL)) {
            temp_pdat <- pData(POeset_WL)
            temp_pdat$expression <- exprs(POeset_WL)[input$data_from_html,]
            temp_pdat <- temp_pdat[order(temp_pdat$time_point),]
            temp_pdat <- temp_pdat[order(temp_pdat$subject),]
            temp_pdat$expression <- scale_values(temp_pdat$expression)
            p2 <- ggboxplot(data = temp_pdat,x = "time_point",y = "expression",add = "dotplot",fill = "time_point",xlab = "years after bariatric surgery",ylab = paste0(input$data_from_html, " expression"), title = "Petrus, P. (2018)") +
              geom_line(aes(group=subject), linetype = "dashed") +
          ...
          if (input$data_from_html %in% rownames(GSE141221_diogenes1_WL)) {
            temp_pdat <- pData(GSE141221_diogenes1_WL)
            temp_pdat$expression <- exprs(GSE141221_diogenes1_WL)[input$data_from_html,]
            temp_pdat <- temp_pdat[order(temp_pdat$time_point),]
            temp_pdat <- temp_pdat[order(temp_pdat$subject),]
            temp_pdat$expression <- scale_values(temp_pdat$expression)
            p3 <- ggboxplot(data = temp_pdat,x = "time_point",y = "expression",fill = "time_point",xlab = "weeks after diet",ylab = paste0(input$data_from_html, " expression"), title = "Imbert, A. (2022)") +
              # geom_line(aes(group=subject), linetype = "dashed") +
          ...
          if (input$data_from_html %in% rownames(GSE95640_diogenes2_WL)) {
            temp_pdat <- pData(GSE95640_diogenes2_WL)
            temp_pdat$expression <- exprs(GSE95640_diogenes2_WL)[input$data_from_html,]
            temp_pdat <- temp_pdat[order(temp_pdat$time_point),]
            temp_pdat <- temp_pdat[order(temp_pdat$subject),]
            temp_pdat$expression <- scale_values(temp_pdat$expression)
            p4 <- ggboxplot(data = temp_pdat,x = "time_point",y = "expression",fill = "time_point",xlab = "weeks after diet",ylab = paste0(input$data_from_html, " expression"), title = "Armenise, C. (2017)") +
              # geom_line(aes(group=subject), linetype = "dashed") +
              stat_compare_means(comparisons = list(c("0","8")), paired = TRUE, method = "wilcox.test") +
              theme(panel.grid.major=element_line(colour=NA),panel.grid.minor=element_line(colour=NA)) +
              theme(panel.border = element_blank()) +
          ...
          # if (input$data_from_html %in% rownames(ADIPOSTRESS_WL)) {
          #   temp_pdat <- pData(ADIPOSTRESS_WL)
          #   temp_pdat$expression <- exprs(ADIPOSTRESS_WL)[input$data_from_html,]
          #   temp_pdat <- temp_pdat[order(temp_pdat$time_point),]
          #   temp_pdat <- temp_pdat[order(temp_pdat$subject),]
          #   p3 <- ggboxplot(data = temp_pdat,x = "time_point",y = "expression",add = "point",fill = "time_point",palette = "ucscgb",xlab = "weeks after diet",facet.by = "group",ylab = paste0(input$data_from_html, " expression"), title = "ADIPOSTRESS weight loss") +
          #     geom_line(aes(group=subject), linetype = "dashed") +
