    ...
    if (gene_reactive()=="") {
      ...
      if (file.exists(paste0('./data/',gene_reactive(),'_RNA.RDS'))) {
        ...
        output$plot2 <- renderPlot({
          ...
          # 先处理SMD值小于-2和大于2的部分
          sex_df_RNA$SMD_binned <- ifelse(sex_df_RNA$SMD < -1, "<-1",
                                      ifelse(sex_df_RNA$SMD > 1, ">1", NA))
          
          # 处理其余部分，将其分成50个等距离的bin
          bins <- 50
          bin_breaks <- seq(-1, 1, length.out = bins + 1)
          sex_df_RNA$SMD_binned[is.na(sex_df_RNA$SMD_binned)] <- cut(sex_df_RNA$SMD[is.na(sex_df_RNA$SMD_binned)], 
                                                             breaks = bin_breaks, include.lowest = TRUE)
          
          # 计算每个bin中的计数
          bin_counts <- table(sex_df_RNA$SMD_binned)
          ...
          bin_data$group <- "out"
          bin_data$group[bin_data$start < sex_df_RNA$SMD[rownames(sex_df_RNA)==gene] & bin_data$end > sex_df_RNA$SMD[rownames(sex_df_RNA)==gene]] <- "in"
        ...
        if (file.exists(paste0('./data/',gene_reactive(),'_protein.RDS'))) {
          ...
          gene_protein <- gene_reactive()
          
          # output$plot3 <- renderPlot({
          # 
          #   df <- rbind(
          #     data.frame(expression = Biobase::exprs(Proteomicseset)[gene_protein, colnames(Proteomicseset)[Proteomicseset$Gender == "m"]], sex = "Male"),
          #     data.frame(expression = Biobase::exprs(Proteomicseset)[gene_protein, colnames(Proteomicseset)[Proteomicseset$Gender == "f"]], sex = "Female")
          #   ) %>% na.omit()
          # 
          #   ggboxplot(data = df, x = "sex", y = "expression", add = "dotplot", fill = "sex", xlab = "", ylab = paste0(gene_protein, " expression")) +
          #     theme(axis.title.x = element_blank(), 
          #           # aspect.ratio = 1,
          #           text = element_text(family = "Red Hat Display"),
          #           legend.position = "none") + 
          #     stat_compare_means(comparisons = list(c("Male", "Female")), method = "wilcox.test") +
          ...
          output$plot4 <- renderPlot({
            ...
            # 先处理SMD值小于-2和大于2的部分
            sex_df_protein$SMD_binned <- ifelse(sex_df_protein$SMD < -1, "<-1",
                                            ifelse(sex_df_protein$SMD > 1, ">1", NA))
            
            # 处理其余部分，将其分成50个等距离的bin
            bins <- 50
            bin_breaks <- seq(-1, 1, length.out = bins + 1)
            sex_df_protein$SMD_binned[is.na(sex_df_protein$SMD_binned)] <- cut(sex_df_protein$SMD[is.na(sex_df_protein$SMD_binned)], 
                                                                       breaks = bin_breaks, include.lowest = TRUE)
            
            # 计算每个bin中的计数
            bin_counts <- table(sex_df_protein$SMD_binned)
            ...
            bin_data$group <- "out"
            bin_data$group[bin_data$start < sex_df_protein$SMD[rownames(sex_df_protein)==gene] & bin_data$end > sex_df_protein$SMD[rownames(sex_df_protein)==gene]] <- "in"
            bin_data$group <- factor(bin_data$group,levels = c("in","out"))
