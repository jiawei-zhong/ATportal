...
shinyServer(function(input, output) {
  ...
  observeEvent(input$start, {
    ...
    if (any(rownames(data_novo_heat) %in% gene_split[[1]])) {
      v$dataNA <- data_novo_heat[rownames(data_novo_heat) %in% gene_split[[1]],,drop=F]
    }
  ...
  observe({
    if (input$cell_line == "hAT MSC"){
      if (!is.null(v$data) & input$prot_switch_adsc == TRUE & any(data_prot_z$gene %in% v$data$gene)) {
        if (input$mode == "normalized"){
    ...
    if (input$cell_line == "SGBS"){
      if (!is.null(v$data) & input$prot_switch_adsc == TRUE) {
        if (input$mode == "normalized"){
  ...
  observe({
    if (!is.null(v$data2)) {
      ...
      v$plot2 <- ggline(data = df, x = "timepoint", y = "expression",numeric.x.axis = F, add = c("mean_se"), color = "gene") +
            theme(text = element_text(family = "Red Hat Display")) +
            scale_color_manual(values = portal_col_ex(length(unique(df$gene))))
  ...
  observe({
    if (!is.null(v$data) & input$cell_line == "hAT MSC") {
      if (input$mode == "normalized") {
        if (nrow(data_niga_heat[rownames(data_niga_heat) %in% v$data$gene,,drop=F]) == 1) {
          v$heatmap <- pheatmap((data_niga_heat[rownames(data_niga_heat) %in% v$data$gene,,drop=F]), 
                                ...
                                labels_col = c("0 minutes","15 minutes","30 minutes","45 minutes","60 minutes",
                                               ...
                                               "12 hours", "24 hours", "48 hours", "4 days", "8 days", "12 days"),silent = T) %>% as.ggplot()
        } else {
          v$heatmap <- pheatmap((data_niga_heat[rownames(data_niga_heat) %in% v$data$gene,,drop=F]), 
    ...
    if (!is.null(v$data) & input$cell_line == "SGBS"){
      if (input$mode == "normalized") {
        if (nrow(data_SGBS_t_heat[rownames(data_SGBS_t_heat) %in% v$data$gene,,drop=F]) == 1) {
          v$heatmap <- pheatmap((data_SGBS_t_heat[rownames(data_SGBS_t_heat) %in% v$data$gene,,drop=F]), 
                                scale = "row", cluster_cols = FALSE, cluster_rows= FALSE, 
                                color = farben[[input$gradient_col]], border_color = NA, fontfamily= "Red Hat Display",
                                main = "Time course SGBS differentiation",
                                labels_col = c("0 hours","24 hours","48 hours","96 hours","192 hours",
                                               "384 hours"),silent = T) %>% as.ggplot()
        } else {
          v$heatmap <- pheatmap((data_SGBS_t_heat[rownames(data_SGBS_t_heat) %in% v$data$gene,,drop=F]), 
                                scale = "row", cluster_cols = FALSE, 
                                color = farben[[input$gradient_col]], clustering_method = input$cluster_method, fontfamily= "Red Hat Display",
  ...
  observe({
    if (!is.null(v$data) & input$cell_line == "hAT MSC" & input$prot_switch_adsc == TRUE  & any(rownames(data_prot_heat) %in% v$data$gene) > 0) {
      if (input$mode == "normalized") {
        if (nrow(data_prot_heat[rownames(data_prot_heat) %in% v$data$gene,]) == 1) {
          v$heatmap_prot <- pheatmap((data_prot_heat[rownames(data_prot_heat) %in% v$data$gene,,drop=F]), 
                                ...
                                labels_col = c("-2 days","0 days","1 day","2 days","4 days",
                                               "6 days","8 days", "10 days", "12 days"),silent = T) %>% as.ggplot()
        } else {
          v$heatmap_prot <- pheatmap((data_prot_heat[rownames(data_prot_heat) %in% v$data$gene,,drop=F]), 
                                scale = "row", cluster_cols = FALSE, 
                                color = farben[[input$gradient_col]], clustering_method = input$cluster_method, fontfamily= "Red Hat Display",
        ...
        if (nrow(data_prot_heat[rownames(data_prot_heat) %in% v$data$gene,,drop=F]) == 1) {
          v$heatmap_prot <- pheatmap((data_prot_heat[rownames(data_prot_heat) %in% v$data$gene,,drop=F]), 
                                cluster_cols = FALSE, cluster_rows= FALSE, 
                                color = farben[[input$gradient_col]], border_color = NA, fontfamily= "Red Hat Display",
                                main = "Time course hADSC differentiation",
    ...
    if (!is.null(v$data) & input$cell_line == "SGBS" & input$prot_switch_adsc == TRUE) {
      if (input$mode == "normalized") {
        if (nrow(data_SGBS_p_heat[rownames(data_SGBS_p_heat) %in% v$data$gene,,drop=F]) == 1) {
          v$heatmap_prot <- pheatmap((data_SGBS_p_heat[rownames(data_SGBS_p_heat) %in% v$data$gene,]), 
                                     ...
                                     labels_col = c("0 days","2 days","4 days","6 days",
                                                    "10 days","12 days", "14 days", "20 days"),silent = T) %>% as.ggplot()
        } else {
          v$heatmap_prot <- pheatmap((data_SGBS_p_heat[rownames(data_SGBS_p_heat) %in% v$data$gene,,drop=F]), 
                                     scale = "row", cluster_cols = FALSE, 
                                     color = farben[[input$gradient_col]], clustering_method = input$cluster_method, fontfamily= "Red Hat Display",
                                     border_color = NA, main = "Time course hADSC differentiation",
                                     labels_col = c("0 days","2 days","4 days","6 days",
  ...
  observe({
    if (!is.null(v$data2)) {
      if (input$mode == "normalized") {
        if (nrow(data_SVF_heat[rownames(data_SVF_heat) %in% v$data2$gene,,drop=F]) == 1) {
          v$heatmap2 <- pheatmap((data_SVF_heat[rownames(data_SVF_heat) %in% v$data2$gene,,drop=F]), 
                                 cluster_cols = FALSE, cluster_rows= FALSE, scale="row", 
                                 color = farben[[input$gradient_col]], border_color = NA, fontfamily= "Red Hat Display",
                                 main = "Time course SVF cells",silent = T) %>% as.ggplot()
        } else {
          v$heatmap2 <- pheatmap((data_SVF_heat[rownames(data_SVF_heat) %in% v$data2$gene,,drop=F]), 
                                 scale = "row", cluster_cols = FALSE, 
                                 color = farben[[input$gradient_col]], clustering_method = input$cluster_method, fontfamily= "Red Hat Display",
                                 border_color = NA, main = "Time course SVF cells",silent = T) %>% as.ggplot()
        }
      } else {
        if (nrow(data_SVF_heat[rownames(data_SVF_heat) %in% v$data2$gene,,drop=F]) == 1) {
          v$heatmap2 <- pheatmap((data_SVF_heat[rownames(data_SVF_heat) %in% v$data2$gene,,drop=F]), 
  ...
  observe({
    if (!is.null(v$dataNA)) {
      v$heatmap_novo <- pheatmap(v$dataNA, 
                                 cluster_cols = T, cluster_rows= (nrow(v$dataNA)!=1), scale=ifelse(test = input$mode == "normalized",yes = "row",no = "none"), 
                                 color = farben[[input$gradient_col]], border_color = NA, fontfamily= "Red Hat Display",
                                 main = "FACS sorted WAT celltypes",
                                 silent = T) %>% as.ggplot()
      
      # if (input$mode == "normalized") {
      #   if (nrow(data_novo_heat[rownames(data_novo_heat) %in% v$dataNA$transcriptID,1:8,drop=F]) == 1) {
      #     v$heatmap_novo <- pheatmap((data_novo_heat[rownames(data_novo_heat) %in% v$dataNA$transcriptID,1:8,drop=F]), 
      #                                cluster_cols = FALSE, cluster_rows= FALSE, scale="row", 
      #                                color = farben[[input$gradient_col]], border_color = NA, fontfamily= "Red Hat Display",
      #                                main = "FACS sorted WAT celltypes", 
  ...
  # proteome WAT fraction
  observe({
    if (!is.null(v$dataNA) & input$prot_switch_fraction == TRUE & any(rownames(data_WATfraction_prot_heat) %in% rownames(v$dataNA))) {
        if (input$mode == "normalized"){
          # v$plot_p <- ggplot(data = data_prot_z[data_prot_z$gene %in% rownames(v$dataNA),], aes(x = timepoint, y = value, group = gene, color = gene)) + 
          #   geom_point() + stat_summary(fun = "mean", geom = "line", aes(group = gene)) + theme_classic() +
          #   scale_color_manual(values = color_vector()[1:num_unique_genes()])
          v$heatmap_WATfraction_prot <- pheatmap(mat = data_WATfraction_prot_heat_normalization[rownames(data_WATfraction_prot_heat_normalization) %in% rownames(v$dataNA),,drop=F], 
                                                 cluster_cols = T, cluster_rows= F, scale="none", 
                                                 color = farben[[input$gradient_col]], border_color = NA, fontfamily= "Red Hat Display",
                                                 # main = "FACS sorted WAT celltypes",
                                                 silent = T) %>% as.ggplot()
        } else {
          # v$plot_p <- ggplot(data = data_prot_raw[data_prot_raw$gene %in% rownames(v$dataNA),], aes(x = timepoint, y = value, group = gene, color = gene)) + 
          #   geom_point()+ stat_summary(fun = "mean", geom = "line", aes(group = gene)) + theme_classic() +
          #   scale_color_manual(values = color_vector()[1:num_unique_genes()])
          v$heatmap_WATfraction_prot <- pheatmap(mat = data_WATfraction_prot_heat[rownames(data_WATfraction_prot_heat) %in% rownames(v$dataNA),,drop=F], 
                                                 cluster_cols = T, cluster_rows= (nrow(data_WATfraction_prot_heat[rownames(data_WATfraction_prot_heat) %in% rownames(v$dataNA),,drop=F])!=1), scale="none", 
                                                 color = farben[[input$gradient_col]], border_color = NA, fontfamily= "Red Hat Display",
                                                 # main = "Proteomics",
                                                 silent = T) %>% as.ggplot()
        }
      } else {
        v$heatmap_WATfraction_prot <- NULL  
      }
  })
  ...
  output$ui_niga_line <- renderUI(
    if (!is.null(v$plot) & !is.null(v$plot_p)) {
      fluidRow(
        column(width = 6, 
    ...
    } else if (!is.null(v$plot) & is.null(v$plot_p)) {
      fluidRow(
        column(width = 12, 
  ...
  output$ui_niga_heat <- renderUI(
    if (!is.null(v$heatmap) & !is.null(v$heatmap_prot)) {
      fluidRow(
        column(width = 6, 
               # style = 'padding-left:0px; padding-right:15px; padding-top:0px; padding-bottom:0px',
               "Transcriptomics",
        ...
        column(width = 6, 
               # style='padding-left:15px; padding-right:0px; padding-top:0px; padding-bottom:0px',
               "Proteomics",
               plotOutput("heatmap_prot"),
               downloadButton(outputId = "download_pdf10", label = "Download PDF", class = "butt"),
               downloadButton(outputId = "download_gg10", label = "Download ggplot2", class = "butt"),
               tags$head(tags$style(".butt {background-color:  #1a4659;} .butt{color: #E2C744;} .butt{border-color: #E2C744;}"))
               
        )
      )
    } else if (!is.null(v$heatmap) & is.null(v$heatmap_prot)) {
      fluidRow(
        column(width = 12, 
               # style = 'padding-left:0px; padding-right:0px; padding-top:0px; padding-bottom:0px',
               plotOutput("heatmap"),
  ...
  output$ui_novo_heat <- renderUI(
    if (!is.null(v$heatmap_novo) & !is.null(v$heatmap_WATfraction_prot)) {
      fluidRow(
        column(width = 6, 
               # style = 'padding-left:0px; padding-right:15px; padding-top:0px; padding-bottom:0px',
               "Transcriptomics",
               plotOutput("heatmap_novo"),
        ...
        column(width = 6, 
               # style='padding-left:15px; padding-right:0px; padding-top:0px; padding-bottom:0px',
               "Proteomics",
               plotOutput("heatmap_WATfraction_prot"),
               downloadButton(outputId = "download_pdf11", label = "Download PDF", class = "butt"),
               downloadButton(outputId = "download_gg11", label = "Download ggplot2", class = "butt"),
               tags$head(tags$style(".butt {background-color:  #1a4659;} .butt{color: #E2C744;} .butt{border-color: #E2C744;}"))
               
        )
      )
    } else if (!is.null(v$heatmap_novo) & is.null(v$heatmap_WATfraction_prot)) {
      fluidRow(
        column(width = 12, 
               # style = 'padding-left:0px; padding-right:0px; padding-top:0px; padding-bottom:0px',
               plotOutput("heatmap_novo"),
  ...
  # Define WAT fractionation heatmap
  output$heatmap_WATfraction_prot <- renderPlot({
    if (!is.null(v$heatmap_WATfraction_prot)) v$heatmap_WATfraction_prot
  })
  ...
  output$table_novo <- DT::renderDataTable({
    if (!is.null(v$dataNA)) {
      datatable(v$dataNA, extensions = 'Buttons', 
                options = list(dom = 'Bfrtip', buttons = c('copy', 'csv', 'excel', 'pdf', 'print')) ) %>% 
        formatRound(columns = colnames(v$dataNA), digits=3)
    }
  })
  
  output$table_WATfraction_prot <- DT::renderDataTable({
    if (!is.null(v$heatmap_WATfraction_prot)) {
      datatable(data_WATfraction_prot_heat, extensions = 'Buttons', 
                options = list(dom = 'Bfrtip', buttons = c('copy', 'csv', 'excel', 'pdf', 'print')) ) %>% 
        formatRound(columns = colnames(data_WATfraction_prot_heat), digits=3)
    }
  })
  ...
  output$download_pdf9 <- downloadHandler(
    filename = function() { paste('tissue_Line_Prot_', Sys.Date(), '.pdf', sep='') },
    content = function(file) { ggsave(file, plot = v$linechart_prot, device = cairo_pdf) }
  )
  
  output$download_pdf10 <- downloadHandler(
    filename = function() { paste('tissue_Heat_Prot_', Sys.Date(), '.pdf', sep='') },
    content = function(file) { ggsave(file, plot = v$heatmap_prot, device = cairo_pdf) }
  )
  
  output$download_pdf11 <- downloadHandler(
    filename = function() { paste('tissue_Heat_Prot_', Sys.Date(), '.pdf', sep='') },
    content = function(file) { ggsave(file, plot = v$heatmap_WATfraction_prot, device = cairo_pdf) }
  )
  ...
  output$download_gg9 <- downloadHandler(
    filename = function() { paste('tissue_Line_Prot_', Sys.Date(), '.rds', sep='') },
    content = function(file) { saveRDS(v$linechart_prot, file) }
  )
  
  output$download_gg10 <- downloadHandler(
    filename = function() { paste('tissue_Proteomics_', Sys.Date(), '.rds', sep='') },
    content = function(file) { saveRDS(v$heatmap_prot, file) }
  )
  
  output$download_gg11 <- downloadHandler(
    filename = function() { paste('tissue_Proteomics_', Sys.Date(), '.rds', sep='') },
    content = function(file) { saveRDS(v$heatmap_WATfraction_prot, file) }
  )
